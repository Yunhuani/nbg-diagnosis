...
3. 核心任务是找出:客户只盯着最显眼那支(拼价格的有框/通用五金),却忽略了旁边更适合自己、利润更高的那支(无框/高端饰面/中东高端工程)。
...
【必须做的分析动作】
1. 用波特五力判断这个竞争环境好不好打:高端品牌壁垒、低端价格竞争、中端国产品牌夹击等。
2. 用蓝海客户价值曲线判断你打得怎么样:客户看重价格、交付稳定性、可定制化、认证齐全度时,你的强弱在哪里。
...
5. 若缺 self_scores 或 unique_assets,必须降级;本次甬辉数据提供了 unique_assets,应认真用于反转判断。

【反转发现:认真做,但不得硬凑】
尝试这个反转方向:客户视为软肋的"大客户依赖(集中度65%)",是否其实是被深度绑定的高切换成本壁垒?
- naive_reading: 大客户依赖、前三大集中度高,常识上看是风险。
- reframe: 若认证、合作年限、定制适配和稳定交付形成绑定,它可能是竞争壁垒,不是单纯软肋。
- mechanism 必须依赖 unique_assets 中的认证/合作年限/定制适配/稳定大批量交付能力。
- falsifier 必须说明什么情况下反转不成立,例如大客户正在询价竞品、认证已被同行普遍获得、合作可随时切换、定制适配不深。
