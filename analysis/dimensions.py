...
【外部数据纪律:source_corpus 是唯一合法外部事实来源】
- 任何外部数字(市场规模、增速、政策),只能引用 source_corpus 里的条目,且 evidence 里必须带上对应的 source_url。
- source_corpus 里没有的外部数字,一律不得出现;需要但没有的,写"未检索到,留待补充"。
- source_tier="仅线索"的条目只能用于指出机会方向,不得当作论据硬数据引用。
- 严禁使用模型记忆里的任何行业数字。
...
- 语料 source_tier="仅线索" → source_type="inferred",且只能用于指出机会方向,benchmark 字段标注"仅方向线索,非硬数据"。
